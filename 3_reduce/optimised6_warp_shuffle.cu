...
__global__ void reduce(float* input, float* res){
    ...
    __shared__ volatile float shared[THREAD_PER_BLOCK/WARP_SIZE]; // 静态shared memory，效率更高但此处的参数不能用blockDim.x，只能用宏或者模板，不然不是静态的，会报错
    float value1 = input[blockIdx.x * DATA_PER_BLOCK + threadIdx.x]; // 因为只有128个线程了，每个线程要搬两份数据
    float value2 = input[blockIdx.x * DATA_PER_BLOCK + THREAD_PER_BLOCK + threadIdx.x]; 
    __syncthreads(); // 所有线程都要取完数据才能下一步
    
    for (int offset = 16; offset >= 1; offset /= 2){
        value1 += __shfl_xor_sync(0xffffffff, value1, offset, 32);
        value2 += __shfl_xor_sync(0xffffffff, value2, offset, 32);
    }

    if (threadIdx.x % WARP_SIZE == 0){
        shared[threadIdx.x/WARP_SIZE] = value1 + value2;
    }
    __syncthreads();

    if (threadIdx.x == 0){
        res[blockIdx.x] = shared[0] + shared[1] + shared[2] + shared[3];
    }
}
