...
#define DATA_PER_BLOCK 512
#define THREAD_PER_BLOCK 256
__global__ void reduce(float* input, float* res){
    
    __shared__ float shared[DATA_PER_BLOCK]; // 静态shared memory，效率更高但此处的参数不能用blockDim.x，只能用宏或者模板，不然不是静态的，会报错
    shared[threadIdx.x] = input[blockIdx.x * DATA_PER_BLOCK + threadIdx.x]; // 增加一个block内的数据量，扩充为原本的两倍
    ...
    for (int count = 1; count <= THREAD_PER_BLOCK; count *= 2){
        int offset = THREAD_PER_BLOCK / count;
