...
__global__ void reduce(float* input, float* res){
    ...
    __shared__ float shared[THREAD_PER_BLOCK]; // 静态shared memory，效率更高但此处的参数不能用blockDim.x，只能用宏或者模板，不然不是静态的，会报错
    shared[threadIdx.x] = input[blockIdx.x * blockDim.x + threadIdx.x]; // 一个block内共用shared memory
    __syncthreads(); // 所有线程都要取完数据才能下一步

    for (int count = 1; count < blockDim.x; count *= 2){
        int offset = blockDim.x / (count * 2);
        if (threadIdx.x < offset){
            shared[threadIdx.x * count * 2] += shared[threadIdx.x * count * 2 + count];
        }
        __syncthreads();
    }
    if (threadIdx.x == 0){
...
int main() {
    // const int THREAD_PER_BLOCK = 1 << 10;
    const int N = 1 << 20;
    ...
    float input[N], result[N / THREAD_PER_BLOCK];
    float resultHost[N / THREAD_PER_BLOCK];
    float blockRes = 0;
    int assignCount = 0;
    int resIndex = 0;
    for (size_t i = 0; i < N; i++){
        ...
        if (assignCount % THREAD_PER_BLOCK == 0){
            resultHost[resIndex] = blockRes;
            blockRes = 0;
            resIndex += 1;
        }
    }
    // for (size_t i = 0; i < N / THREAD_PER_BLOCK; i++){
    //     printf("result from host at %zu is %f \n", i, resultHost[i]);
    // }
    // 分配设备内存
    float *d_input, *d_res;
    cudaMalloc((void**)&d_input, nBytes);
    cudaMalloc((void**)&d_res, nBytes / THREAD_PER_BLOCK);
    ...
    // 拷贝device侧结果回host
    cudaMemcpy((float*)result, d_res, nBytes / THREAD_PER_BLOCK, cudaMemcpyDeviceToHost);
