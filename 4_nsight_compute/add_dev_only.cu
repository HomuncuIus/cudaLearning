...
__global__ void add_align(float* a, float*b, float* res){
...
__global__ void add_unalign(float* a, float*b, float* res){
...
int main(){
    ...
    float *host_a = (float *)malloc(NByte);
    float *host_b = (float *)malloc(NByte);
    ...
    dim3 grid(N / (1 << 8));
    dim3 block(64);
    add_align<<<grid, block>>>(dev_a, dev_b, dev_res);
    cudaDeviceSynchronize();

    add_unalign<<<grid, block>>>(dev_a, dev_b, dev_res);
    cudaDeviceSynchronize();
